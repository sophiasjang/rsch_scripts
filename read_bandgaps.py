#!/usr/bin/python
'''
reads bandgaps from anharmonicity check claculations and writes them to a two
...
def read_bandgap(directory, mode_n):
    print
            ...
            if m:
                ...
                for line in fin:
                    if 'highest occupied, lowest unoccupied level (ev):' in line:
                        homo, lumo = map(float, line.split()[-2:])
                        scale_vs_bandgap.append((float(s), lumo-homo))
                        break
                else:
                    print '  No convergence for scale {0}.'.format(s)
                break
        else:
            print '  Warning: No matching .out file found in ./scale{1}'.format(mode_n, s)
    
    os.chdir(root)
    
    if scale_vs_bandgap:
        
        fn_out = os.path.join(mode_dir, fn_base + '_GvA.dat')
        print '  Writing {0}.'.format(os.path.basename(fn_out))
        f = open(fn_out, 'w')
        for s, bg in sorted(scale_vs_bandgap):
            f.write('{0}  {1}\n'.format(s, bg))
        return os.path.relpath(fn_out)
    else:
        print '  No bandgaps read.'
        return ''
    
def usage():
    print
    print 'Reads bandgaps from anharmonicity check calculations and writes'\
           + ' them to a two\ncolumn file *_GvA.dat'
    print
    ...
    for d in os.listdir(os.getcwd()):
        if os.path.isdir(d) and d.startswith('mode'):
            ...
            if modes == [] or mode in modes:
                files.append(read_bandgap(d, mode))
                done.append(mode)
